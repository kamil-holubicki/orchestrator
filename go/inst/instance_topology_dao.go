...
func EnableMasterGetSourcePublicKey(instanceKey *InstanceKey) (*Instance, error) {
	...
	log.Debugf("EnableMasterSSL: Will attempt enabling GetSourcePublicKey replication on %+v", *instanceKey)

	if *config.RuntimeCLIFlags.Noop {
		return instance, fmt.Errorf("noop: aborting CHANGE MASTER TO GET_SOURCE_PUBLIC_KEY=1 operation on %+v; signaling error but nothing went wrong.", *instanceKey)
	}
	...
	log.Infof("EnableMasterSSL: Enabled GetSourcePublicKey replication on %+v", *instanceKey)
